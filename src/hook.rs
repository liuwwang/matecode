...
pub async fn install_post_commit_hook() -> Result<()> {
    let hook_path = get_hook_path().await?;
    let hooks_dir = hook_path.parent().context("Failed to get hooks directory from path")?;
    ...
    if hook_path.exists() {
        ...
        fs::write(&hook_path, new_content).await.context("Failed to append to post-commit hook")?;
